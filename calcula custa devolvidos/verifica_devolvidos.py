...
def gravar_tabela(custas, creditos):
    ...
    with open(f'custas_resultado/custas_{data}{hora}.csv', 'w') as saida:
        ...
        for item in creditos:
            saida.write(f'{item}\n')
        saida.write('\n\nemol;iss;faadep;frmmp;ferm;selo;dist\n')
        novo = ['{:.2f}'.format(x / 100).replace('.',',') for x in custas]
        custa = ';'.join(novo)
        saida.write(custa)

def soma_custas(lista, tabela, edital):
    soma_total = [0]
    ...
    for item in lista:
        for custa in tabela[1:]:
            ...
            if sum(custa_arr) == int(item):
                soma_array(soma_total, custa_arr[:7])
                edital_arr = [int(x) for x in edital.split(';')]
...
def main():
    ...
    edital = _arquivo.abrir('config/edital.txt')
        ...
        if mapa:
            for codigo in mapa:
                for nome in mapa[codigo]:
                    ...
                    nao_conciliados.append([ant_codigo, indice, root_path])
                    devolvidos_geral[indice] = prots_csv
    ...
    custas = soma_custas(custas_totais, tabela, edital[0])
    
    if creditos:
        gravar_tabela(custas, creditos)

    falta_pagar = []
    nao_deletar = []

    for item in nao_conciliados:
        if item[1] not in chaves:
            if ';0,00' not in item[1]:
                falta_pagar.append('{:50s}{}   {}'.format(item[1], item[0], item[2]))
