...
class GeraTed:
    ...
    def cria_seguimento_o(self, dados):
        valor_total = 0
        conteudo = []
        cont = 1
        for item in dados:
            dado = item.split(';')
            segO = self.modelo[6]
            segO = segO.replace(self.mapa[2],self.comp_zeros(self.mapa[2], str(cont)))
            segO = segO.replace(self.mapa[19],dado[1])
            segO = segO.replace(self.mapa[20],self.comp_espacos(self.mapa[20], dado[2].upper()))
            segO = segO.replace(self.mapa[21],dado[3])
            segO = segO.replace(self.mapa[22],dado[4])
            valor = dado[5].replace('.','').replace(',','').strip()
            segO = segO.replace(self.mapa[23],self.comp_zeros(self.mapa[23], valor))
            cont += 1
            valor_total += int(valor)
            conteudo.append(segO)
        return [conteudo, valor_total]
